...
%we can interpret traffic equilibrium as a differential game in which Demon proposes a traffic flow $\mathbf{y}$. This flow induces a travel cost across each link in the network, represented by $\mathbf{t}(\mathbf{y})$. Angel tries to propose an alternate traffic flow $\mathbf{z}$ which, using Demons link costs $\mathbf{t}(\mathbf{y})$, results in a lower total cost in the network.
